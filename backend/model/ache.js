...
const getTotalCount = (params) => {
  let sql = `SELECT COUNT(*) AS TOTAL FROM achievement where ${params.type === 'all' ? '' : 'type = ? and'} userId = ?`
...
module.exports = {
  getList: async (params, res, rej) => {
    ...
    let sql = `SELECT * FROM achievement where ${params.type === 'all' ? '' : 'type = ? and'} userId = ? limit ?,?`
    let values = [params.openid, params.current * params.pageSize, params.pageSize]
    if (params.type !== 'all') {
      values = [params.type].concat(values)
    }
    let query = {
