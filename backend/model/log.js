...
module.exports = {
  getList: async (params, res, rej) => {
    ...
    let query = {
      ...
      timeout: 4000,
      values: [params.openid, params.current * params.pageSize, params.pageSize]
    }
    db.row(query).then(
  ...
  addLog: (params, res, rej) => {
    ...
    let query = {
      sql: 'INSERT INTO log(type, name, point, updateTime, calc, userId) VALUE (? ,?, ?, ?, ?, ?)',
      timeout: 4000,
      values: [params.refresh, params.name, params.point, time, params.calc, params.openid]
    }
    db.row(query).then(
      (dbRes) => res(dbRes),
      (err) => rej(err)
